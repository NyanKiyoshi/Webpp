...
if (mode!=0)
 ...
 switch (mode[0])
  ...
  case 'a':
   i->Set("Mode","b");
   i->HiddenDump();
   // Note the use of the newer flashy system in the two lines above
   // rather than the two commented out below.
  ...
  case 'b':
   //i->HiddenDump();
   //printf("Setting... ");
   i->Set("Mode","StartAgain");
   i->HiddenDump();
   // Just for kicks again thought this brings rise the thought to making
   // the hidden type a member of the variable rather than a dump routine.
